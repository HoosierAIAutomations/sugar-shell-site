...
const BrownieBitesModal = ({ product, onClose, onAddToCart, orderingOpen }) => {
    ...
    const pairingCost = pairings.reduce((total, p) => {
        const drizzle = DRIZZLE_OPTIONS.find(d => d.id === p.drizzle);
        const topping = TOPPING_OPTIONS.find(t => t.id === p.topping);
        return total + (drizzle?.price || 0) + (topping?.price || 0);
    }, 0);

    // Deduplicate: a pairing that is "no drizzle + no topping" costs $0 and is effectively free
    // Each non-free selection costs $1 as per spec: "$1 extra per pairing"
    // Re-interpret: each PAIRING SLOT beyond the free base costs $1 if it has any non-"no" selection
    ...
    const handleAddToCart = () => {
        const cartItem = {
            ...
            id: `brownie-bites-box--${pairings.map(p => `${p.drizzle}:${p.topping}`).join('|')}`,
            price: totalPrice,
                ...
                <div className="modal-body">
                    <div className="pairings-header">
                        ...
                        <p className="pairings-hint">
                            Add up to {product.maxPairings} drizzle &amp; topping combos · <strong>$1 per pairing</strong>
                        </p>
                    </div>
